function CreateNBUISettings()
	...
	-- Copy mail to new page keybind.
	table.insert(optionsData, {
		type = "dropdown",
		name = GetString(SI_NBUI_COPYMAIL_NAME),
		tooltip = GetString(SI_NBUI_COPYMAIL_TOOLTIP),
		choices = NBUI.MailToPageOptions,
		choicesValues = NBUI.MailToPageOptions,
		getFunc = function() return NBUI.db.NB1_MailToPage end,
		setFunc = function(v)
			NBUI.db.NB1_MailToPage = v
			-- Apply.
			for i=1, #MAIL_INBOX.selectionKeybindStripDescriptor do
				local item = MAIL_INBOX.selectionKeybindStripDescriptor[i]
				if item.name == GetString(SI_NBUI_COPYMAIL_NAME) then
					item.keybind = v
					break
				end
			end
