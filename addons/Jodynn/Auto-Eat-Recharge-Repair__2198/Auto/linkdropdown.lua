--[[linkdropdownData = {
    ...
    scrollable = true, -- boolean or number, if set the linkdropdown will feature a scroll bar if there are a large amount of choices and limit the visible lines to the specified number or 10 if true is used (optional)
...
if not LAM:RegisterWidget("linkdropdown", widgetVersion) then return end
...
local function UpdateDisabled(control)
    ...
    control.linkdropdown:SetEnabled(not disable)
    if disable then
...
local function UpdateValue(control, forceDefault, value)
    if forceDefault then --if we are forcing defaults
        ...
        control.linkdropdown:SetSelectedItem(control.choices[value])
    elseif value then
        control.data.setFunc(value)
        --after setting this value, lets refresh the others to see if any should be disabled or have their settings changed
...
itemtool = ItemTooltip

local function SetupTooltips(comboBox, choicesTooltips)
    local function ShowTooltip(control)
        InitializeTooltip(itemtool, control, TOPLEFT, 0, 0, BOTTOMRIGHT)
        itemtool:SetLink(LAM.util.GetStringFromValue(control.tooltip))
    end
    local function HideTooltip(control)
        ClearTooltip(itemtool)
    end
...
local function UpdateChoices(control, choices, choicesValues, choicesTooltips)
    control.linkdropdown:ClearItems() --remove previous choices --(need to call :SetSelectedItem()?)
    ...
    if choicesTooltips then
        ...
        if not control.scrollHelper then -- only do this for non-scrollable
            SetupTooltips(control.linkdropdown, choicesTooltips)
        end
    end

    for i = 1, #choices do
        local entry = control.linkdropdown:CreateItemEntry(choices[i], DropdownCallback)
        ...
        control.linkdropdown:AddItem(entry, not control.data.sort and ZO_COMBOBOX_SUPRESS_UPDATE) --if sort type/order isn't specified, then don't sort
    end
end
...
local PADDING = GetMenuPadding() / 2 -- half the amount looks closer to the regular linkdropdown
...
function ScrollableDropdownHelper:Initialize(parent, control, visibleRows)
    ...
    local linkdropdown = control.linkdropdown
    self.parent = parent
    self.control = control
    self.combobox = combobox
    self.linkdropdown = linkdropdown
    self.visibleRows = visibleRows

    -- clear anchors so we can adjust the width dynamically
    linkdropdown.m_linkdropdown:ClearAnchors()
    linkdropdown.m_linkdropdown:SetAnchor(TOPLEFT, combobox, BOTTOMLEFT)

    -- handle linkdropdown or settingsmenu opening/closing
    local function onShow() self:OnShow() end
    local function onHide() self:OnHide() end
    local function doHide() self:DoHide() end

    ZO_PreHook(linkdropdown, "ShowDropdownOnMouseUp", onShow)
    ZO_PreHook(linkdropdown, "HideDropdownInternal", onHide)
    ...
    local scrollList = linkdropdown.m_scroll
    ...
    local scroll = linkdropdown.m_linkdropdown:GetNamedChild("Scroll")
    ...
    local dataType1 = ZO_ScrollList_GetDataTypeTable(linkdropdown.m_scroll, 1)
    local dataType2 = ZO_ScrollList_GetDataTypeTable(linkdropdown.m_scroll, 2)
    ...
    local function SetupEntry(control, data, list)
        ...
        -- no need to store old ones since we have full ownership of our linkdropdown controls
    ...
    ZO_PreHook(linkdropdown, "AddMenuItems", function()
        local width = PADDING * 2 + zo_max(self:GetMaxWidth(), combobox:GetWidth())
        local numItems = #linkdropdown.m_sortedItems
        ...
        local height = PADDING * 2 + numItems * (SCROLLABLE_ENTRY_TEMPLATE_HEIGHT + linkdropdown.m_spacing) - linkdropdown.m_spacing + ROUNDING_MARGIN
        linkdropdown.m_linkdropdown:SetWidth(width)
        linkdropdown.m_linkdropdown:SetHeight(height)
    end)
end

function ScrollableDropdownHelper:OnShow()
    local linkdropdown = self.linkdropdown
    if linkdropdown.m_lastParent ~= ZO_Menus then
        linkdropdown.m_lastParent = linkdropdown.m_linkdropdown:GetParent()
        linkdropdown.m_linkdropdown:SetParent(ZO_Menus)
...
function ScrollableDropdownHelper:OnHide()
    local linkdropdown = self.linkdropdown
    if linkdropdown.m_lastParent then 
        linkdropdown.m_linkdropdown:SetParent(linkdropdown.m_lastParent)
        linkdropdown.m_lastParent = nil
    end
end

function ScrollableDropdownHelper:DoHide()
    local linkdropdown = self.linkdropdown
    if linkdropdown:IsDropdownVisible() then
        linkdropdown:HideDropdown()
    end
end

function ScrollableDropdownHelper:GetMaxWidth()
    local linkdropdown = self.linkdropdown
    local dataType = ZO_ScrollList_GetDataTypeTable(linkdropdown.m_scroll, 1) 

    local dummy = dataType.pool:AcquireObject()
    dataType.setupCallback(dummy, {
        m_owner = linkdropdown,
        name = "Dummy"
    }, linkdropdown)

    local maxWidth = 0
    local label = dummy.m_label
    local entries = linkdropdown.m_sortedItems
...
function LAMCreateControl.linkdropdown(parent, linkdropdownData, controlName)
    local control = LAM.util.CreateLabelAndContainerControl(parent, linkdropdownData, controlName)
    control.choices = {}
    ...
    control.combobox = wm:CreateControlFromVirtual(zo_strjoin(nil, name, "Combobox", comboboxCount), control.container, linkdropdownData.scrollable and "ZO_ScrollableComboBox" or "ZO_ComboBox")
    ...
    control.linkdropdown = ZO_ComboBox_ObjectFromContainer(combobox)
    local linkdropdown = control.linkdropdown
    linkdropdown:SetSortsItems(false) -- need to sort ourselves in order to be able to sort by value

    if linkdropdownData.scrollable then
        local visibleRows = type(linkdropdownData.scrollable) == "number" and linkdropdownData.scrollable or DEFAULT_VISIBLE_ROWS 
        control.scrollHelper = ScrollableDropdownHelper:New(parent, control, visibleRows)
    end

    ZO_PreHook(linkdropdown, "UpdateItems", function(self)
        assert(not self.m_sortsItems, "built-in linkdropdown sorting was reactivated, sorting is handled by LAM")
    ...
    if linkdropdownData.sort then
        local sortInfo = GrabSortingInfo(linkdropdownData.sort)
        control.m_sortType, control.m_sortOrder = SORT_TYPES[sortInfo[1]], SORT_ORDERS[sortInfo[2]]
    elseif linkdropdownData.choicesValues then
        control.m_sortType, control.m_sortOrder = ZO_SORT_ORDER_UP, SORT_BY_VALUE
    end

    if linkdropdownData.warning ~= nil or linkdropdownData.requiresReload then
    ...
    control:UpdateChoices(linkdropdownData.choices, linkdropdownData.choicesValues)
    control.UpdateValue = UpdateValue
    control:UpdateValue()
    if linkdropdownData.disabled ~= nil then
